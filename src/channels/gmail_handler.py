...
class GmailHandler:
    ...
    async def send_reply(self, thread_id: str, content: str, in_reply_to: str, to_email: str) -> bool:
        ...
        try:
            service = self._get_gmail_service()
            if not service:
                logger.warning("Gmail service not available, using SMTP fallback")
                # Fallback to SMTP sender
                from src.channels.email_sender import get_email_sender
                email_sender = get_email_sender()
                return await email_sender.send_ticket_response(
                    to_email=to_email,
                    ticket_id=thread_id,  # Using thread_id as placeholder
            ...
            message.attach(MIMEText(self._create_html_reply(content), "html", "utf-8"))
